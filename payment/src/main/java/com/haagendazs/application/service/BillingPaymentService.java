...
public class BillingPaymentService {
    ...
    private void payWithBillingMethod(
        ...
        if (billing != null && !billing.isActive()) {
            throw new BusinessException(PaymentErrorCode.BILLING_METHOD_NOT_FOUND);
        }
    ...
    private boolean handleTossPaymentException(
        ...
        if (exception.getTossPaymentErrorCode() == TossPaymentErrorCode.INVALID_CARD_EXPIRATION) {
            failBillingPaymentAndExpireBillingMethod(memberId, preparation, exception);
            return false;
        }
    ...
    private void failBillingPaymentAndExpireBillingMethod(
            Long memberId,
            BillingPaymentTransactionService.BillingPaymentPreparation preparation,
            RuntimeException exception
    ) {
        try {
            billingPaymentTransactionService.failBillingPaymentAndExpireBillingMethod(
                    memberId,
                    preparation,
                    exception
            );
            ...
            log.warn(
                    "카드 유효기간 만료 자동결제 실패 후 상태 보정 실패 memberId={} orderNo={}",
                    memberId,
                    preparation.orderNo(),
                    handlingException
            );
